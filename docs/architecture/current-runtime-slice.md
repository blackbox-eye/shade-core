...
- The implemented public slice is still small: run state, confidence and self-model records, worker registry, runtime decision and audit event records, contract-gate and evaluation-gate results, decision and audit helpers, evaluation helpers, serializers, and `build_bundle`.
- The current core implements one internal runtime/evaluation fabric path that prepares state validation, decision, audit, evaluation, evaluation gate, and inspectable snapshot output.
